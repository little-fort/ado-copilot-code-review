...
if ($Id -gt 0) {
    ...
    if ($threads -and $threads.value) {
        ...
        # Match by: Build Service author OR the agent attribution tag. The tag
        # check matters when comments are posted with a PAT — the author is then
        # a human account, so only the tag identifies the thread as agent-created.
        $copilotThreads = $commentThreads | Where-Object {
            ...
            # Check if comment contains an agent attribution tag
            ($_.comments[0].content -like "*[Generated by GitHub Copilot]*") -or
            ($_.comments[0].content -like "*[Generated by Claude Code]*")
        }
